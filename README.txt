-----------------------------------------------
(Japanese)
-----------------------------------------------

【ソフトウェアの名称】Saimin（サイミン）改め「催眠くらくら」

【説明】

このソフトウェアは、特殊な映像と音声により、精神の自己コントロールを応援します。

【対応OS】

- Windows 10以降のOS。

【対応言語】

- 日本語、英語、中国語、ドイツ語、イタリア語、韓国語。

【使い方】

- ファイル「Saimin.exe」をダブルクリックして起動して下さい。
- 詳しい使い方は「？」ボタンを押せば見られます。
- 終了したい場合は、キーボードの[Alt]+[F4]を押して下さい。

【使用上の注意】

- 本ソフトウェアは、ジョークソフトです。
- 医師法により効用をうたうことはできません。
- てんかんの症状のある方は使用しないで下さい。
- 急性期の統合失調症患者は使用禁止です。
- 精神実験には被験者の同意が必要です。
- 使用直後は、自動車の運転を避けて下さい。
- 集合体恐怖症の人は使用しないで下さい。
- 頭痛、吐き気、胃腸の不具合、異常な感情などの症状が生じた場合は、速やかに使用を中止し、専門医の診断を受けて下さい。

【使用許諾と著作権表示】

Saimin はフリーソフトですので、無料で使用できます。

このソフトウェアの使用による影響については作者は責任を一切負いかねます。

Copyright (C) 2022-2024 片山博文MZ. All Rights Reserved.
※ 一部、OtoLogicの音声素材を使用。

【Webアプリ「くらくら」の紹介】

次のURLからWebアプリをインストールした方が使いやすいかもしれません。

- https://katahiromz.github.io/saimin/

【連絡先】

作者へメール：katayama.hirofumi.mz@gmail.com
掲示板：https://katahiromz.bbs.fc2.com/

-----------------------------------------------
(English)
-----------------------------------------------

[(The name of this software)] "Hypnosis KraKra" (old name: Saimin)

[(Description)]

This software supports mental self-control through special video and audio.

[(How to use)]

- Double-click the file "Saimin.exe" to start it.
- For detailed instructions, click the "? button.
- To exit, press [Alt]+[F4] on your keyboard.

[(Supported OSs)]

- Windows 10 or later.

[(Supported Languages)]

- English, Japanese, Chinese, German, Italian, and Korean.

[(Precautions for use)]

- This software is a joke software.
- The benefits of the software cannot be insisted by the Medical Practitioners Act in some countries.
- Do not use this software if you have epilepsy symptoms.
- Acute schizophrenic patients should not use this software.
- Mental experiments require the consent of the subject.
- Avoid driving a car immediately after using this software.
- People with trypophobia should not use this software.
- If you experience symptoms such as headache, nausea, gastrointestinal problems, or abnormal emotions, discontinue use immediately and consult a specialist.

[(License and Copyright Notice)]

Saimin is a free software distributed under the terms of the Apache License Version 2.0, so it is free to use.

The author assumes no responsibility for any consequences resulting from the use of this software.

Copyright (C) 2022-2024 katahiromz. All Rights Reserved.

* Some OtoLogic audio materials are used.

[(Introduction to the Web app "KraKra")]

It may be easier to use if you install the Web app from the following URL:

- https://katahiromz.github.io/saimin/

[(Contact)]

Send email to the author: katayama.hirofumi.mz@gmail.com
-----------------------------------------------
